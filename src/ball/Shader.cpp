...
void ShaderProgram::getUniformIDs() {
	static const char *strUniform[SU_FINISH] = { 
		"transform", "modelview", "normal_mat", "light_pos", "eye_pos", "index", "color", "time", "tex0", "tex1", "tex2", "env_map" };
...
void ShaderProgram::uniform(const char *uid, const vec3 &value) {
	int id = glGetUniformLocation(programID, uid);
...
void ShaderProgram::uniform(const char *uid, const vec4 &value) {
	int id = glGetUniformLocation(programID, uid);
...
void ShaderProgram::uniform(const char *uid, const mat3 &value) {
	int id = glGetUniformLocation(programID, uid);
...
void ShaderProgram::uniform(STD_UNIFORM id, const vec3 &value) {
	if(uniformIDs[id] < 0)
		return;
	glUniform3fv(uniformIDs[id], 1, value);
}

void ShaderProgram::uniform(STD_UNIFORM id, const vec4 &value) {
	if(uniformIDs[id] < 0)
		return;
...
void ShaderProgram::uniform(STD_UNIFORM id, const mat3 &value) {
	if(uniformIDs[id] < 0)
		return;
	glUniformMatrix3fv(uniformIDs[id], 1, GL_FALSE, value);
}
