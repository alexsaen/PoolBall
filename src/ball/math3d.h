...
inline const bool is_NAN(float x) {
	return (*(unsigned int*)&x & 0x7f800000) == 0x7f800000;
}

inline const float absf(float v) {
	unsigned int result = *(unsigned int*) &v;
	result &= 0x7fffffff;
    return *(float*)&result;	
}

inline const float signf(float v) {		// return 1.0 if v>=0, return -1 if v<0
	unsigned int result = *(unsigned int*) &v;
	result = (result & 0x80000000) | 0x3f800000;
    return *(float*)&result;	
}

inline const float fast_sqrtf(float x) {	// 5% calculation error
	unsigned int result = *(unsigned int*)&x;
	result = ( ((result - 0x3f800000) >> 1) + 0x3f800000 ) & 0x7fffffff;
	return *(float*)&result;
}

inline const unsigned clp2(unsigned x) {	// round up to 2^n
	x--;
	x = x | (x >> 1);
	x = x | (x >> 2);
	x = x | (x >> 4);
...
inline const unsigned flp2(unsigned x) {	// round down to 2^n
	x = x | (x >> 1);
	x = x | (x >> 2);
	x = x | (x >> 4);
	x = x | (x >> 8);
...
inline  const float deg2rad(float a)			{ return PI/180.0f*a; }
inline  const float rad2deg(float a)			{ return 180.0f/PI*a; }
...
struct vec2 {
	...
	int operator==(const vec2 &v) const 					{ return (absf(x - v.x) < EPSILON && absf(y - v.y) < EPSILON); }
	int operator!=(const vec2 &v) const						{ return !(*this == v); 			}
	...
	float fast_length() const    							{ return fast_sqrtf(x * x + y * y); }
...
struct vec3 {
	...
	vec3(const vec2 &v, float az) : x(v.x), y(v.y), z(az)					{}
	vec3(const vec3 &v) : x(v.x), y(v.y), z(v.z)     						{}
	vec3(const vec4 &v);
	
	const bool operator==(const vec3 &v) const 								{ return (absf(x - v.x) < EPSILON && absf(y - v.y) < EPSILON && absf(z - v.z) < EPSILON); }
	const bool operator!=(const vec3 &v) const								{ return !(*this == v); 					}
	...
	const float operator*(const vec3 &v) const 								{ return x * v.x + y * v.y + z * v.z; 		}
	...
	const float fast_length() const    										{ return fast_sqrtf(x * x + y * y + z * z); }
	const float length2() const												{ return x * x + y * y + z * z; 			}
...
struct vec4 {
	...
	vec4(const float v) : x(v), y(v), z(v), w(1.0f)                  							{}
	vec4(const vec3 &v) : x(v.x), y(v.y), z(v.z), w(1)               							{}
	vec4(const vec3 &v,float w) : x(v.x), y(v.y), z(v.z), w(w)       							{}
	vec4(const vec4 &v) : x(v.x), y(v.y), z(v.z), w(v.w)             							{}
	
	const bool operator==(const vec4 &v) const						{ return (absf(x - v.x) < EPSILON && absf(y - v.y) < EPSILON && absf(z - v.z) < EPSILON && absf(w - v.w) < EPSILON); }
	const bool operator!=(const vec4 &v) const						{ return !(*this == v); 							}
	...
	const float operator*(const vec4 &v) const 						{ return x * v.x + y * v.y + z * v.z + w * v.w; }
...
struct mat2 {
	...
	const mat2 operator*(const float f) const {
		mat2 ret;
	...
	mat2 &operator*=(const float f) 				{ return *this = *this * f; }
	...
	const float det() const 						{ return mat[0] * mat[3] - mat[1] * mat[2];		}
	...
	void zero() 									{ mat[0] = mat[1] = mat[2] = mat[3] = 0.0f;		}
	...
	static const mat2 get_rotate(float angle) {
        mat2 ret;
        ret.rotate(angle);
        return ret;
	}

	static const mat2 get_scale(const vec2 &v) {
        mat2 ret;
		ret.scale(v);
        return ret;
	}

	static const mat2 get_scale(float x, float y) {
        mat2 ret;
		ret.scale(vec2(x,y));
        return ret;
	}

    static const mat2 get_identity()  {
		return mat2(1.0f);
	}
...
struct mat3 {
	...
	static const mat3 get_rotate(float angle,const vec3 &axis) {
        mat3 ret;
        ret.rotate(angle,axis);
        return ret;
    }

	static const mat3 get_rotate(float angle, float x, float y, float z) {
        mat3 ret;
		ret.rotate(angle,vec3(x,y,z));
        return ret;
	}

	static const mat3 get_rotate_x(float angle) {
        mat3 ret;
        ret.rotate_x(angle);
        return ret;
	}

	static const mat3 get_rotate_y(float angle) {
        mat3 ret;
        ret.rotate_y(angle);
        return ret;
	}

	static const mat3 get_rotate_z(float angle) {
        mat3 ret;
        ret.rotate_z(angle);
        return ret;
	}

	static const mat3 get_scale(const vec3 &v) {
        mat3 ret;
		ret.scale(v);
        return ret;
	}

	static const mat3 get_scale(float x, float y, float z) {
        mat3 ret;
		ret.scale(vec3(x, y, z));
        return ret;
	}

    static const mat3 get_identity()  					{ return mat3(1.0f); 	}
...
struct mat4 {
	...
	const mat4 transpose_rotation() const {
		mat4 ret;
	...
	static const mat4 look_at(const vec3 &eye,const vec3 &dir,const vec3 &up) {
		vec3 x,y,z;
		mat4 m0,m1;
		z = eye - dir;
		z.normalize();
	...
	static const mat4 get_ortho(float left, float right, float bottom, float top, float near=-1, float far=1) {
		mat4 ret;
		ret.ortho(left, right, bottom, top, near, far);
		return ret;
	}

	static const mat4 get_perspective(float fov, float aspect, float znear, float zfar) {
		mat4 ret;
		ret.perspective(fov, aspect, znear, zfar);
		return ret;
	}

    static const mat4 get_translate(const vec3 &v) {
        mat4 ret;
        ret.translate(v);
        return ret;
	}

    static const mat4 get_translate(float x,float y,float z) {
        mat4 ret;
        ret.translate(vec3(x,y,z));
        return ret;
	}

	static const mat4 get_rotate(float angle,const vec3 &axis) {
        mat4 ret;
        ret.rotate(angle,axis);
        return ret;
    }

	static const mat4 get_rotate(float angle,float x,float y,float z) {
        mat4 ret;
		ret.rotate(angle,vec3(x,y,z));
        return ret;
	}

	static const mat4 get_rotate_x(float angle) {
        mat4 ret;
        ret.rotate_x(angle);
        return ret;
	}

	static const mat4 get_rotate_y(float angle) {
        mat4 ret;
        ret.rotate_y(angle);
        return ret;
	}

	static const mat4 get_rotate_z(float angle) {
        mat4 ret;
        ret.rotate_z(angle);
        return ret;
	}

	static const mat4 get_scale(const vec3 &v) {
        mat4 ret;
		ret.scale(v);
        return ret;
	}

	static const mat4 get_scale(float x,float y,float z) {
        mat4 ret;
		ret.scale(vec3(x,y,z));
        return ret;
	}

    static const mat4 get_identity()  					{ return mat4(1.0f); }
...
struct quat {
	...
	static const quat get_rotate(float angle, const vec3 &axis) {
        quat ret;
        ret.set(axis, angle);
        return ret;
    }

	static const quat get_rotate(float angle, float x, float y, float z) {
        quat ret;
		ret.set(vec3(x,y,z), angle);
        return ret;
	}

	static const quat get_rotate_x(float angle) {
        quat ret;
        ret.rotate_x(angle);
        return ret;
	}

	static const quat get_rotate_y(float angle) {
        quat ret;
        ret.rotate_y(angle);
        return ret;
	}

	static const quat get_rotate_z(float angle) {
        quat ret;
        ret.rotate_z(angle);
        return ret;
	}
...
inline const quat slerp(const quat &q0, const quat &q1, float t) {
...
inline const float qot(const quat &q1, const quat &q2) {
	return q1.x*q2.x + q1.y*q2.y + q1.z*q2.z + q1.w*q2.w;
}

inline  const vec3 calcNormal(const vec3 vert[3])                             { vec3 result = cross(vert[0] - vert[1], vert[1] - vert[2]); result.normalize(); return result; }
inline  const vec3 calcNormal(const vec3 &v0, const vec3 &v1, const vec3 &v2) { vec3 result = cross(v0 - v1, v1 - v2); result.normalize(); return result; }
...
struct rect {
	...
	const bool 	inside(float px, float py) const										{ 	return px>=x && px<=x+width && py>=y && py<=y+height; }
	const bool 	intersect(const rect &r) const											{ 	return r.x < x+width && x < r.x+r.width && r.y < y+height && y < r.y+r.height;	}
...
inline const vec3 reflex(const vec3& normalizedVelocity, const vec3 &normal) {
	return  normal*(2.0f*(-normalizedVelocity * normal)) + normalizedVelocity;
}

inline const vec2 reflex(const vec2& normalizedVelocity, const vec2 &normal) {
	return  normal*(2.0f*(-normalizedVelocity*normal)) + normalizedVelocity;
}

inline const bool linesIntersection(const vec2 &tp1, const vec2 &tp2, const vec2 &sc1, const vec2 &sc2, vec2 &intersectionPoint) {
	...
	if( (0 <= a) && (a <= 1) && (0 <= b) && (b <= 1) ) {
		intersectionPoint = tp1 + (tp2 - tp1) * b;
		return true;
	}
	return false;
}
